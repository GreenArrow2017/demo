...
from models_code.CNNmodel_5layers import CNNModel
import os
import sys
...
scheduler = lr_scheduler.ReduceLROnPlateau(optimizer, mode='max', factor=0.5, patience=3, verbose=True, threshold=0.000001, threshold_mode='rel', cooldown=0, min_lr=0, eps=1e-10)#定义存储数据的列表
train_losses = []
train_counter = []
train_acces = []
test_losses = []
...
def test(epoch):
    ...
    f = open("mnist_cifar/run/acc/test_acc_mnist.text", 'w')
    f.write('\n'.join(str_test_accs))
    f.close()
...
for epoch in range(n_epochs+1, continue_epochs+1):
    train(epoch)
    test(epoch)
